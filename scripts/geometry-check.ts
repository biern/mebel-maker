...
state.layoutAnchors.push({ id: state.nextLayoutAnchorId, boardId: shelf.id, axis: "x", offset: -60 });
state.nextLayoutAnchorId += 1;
...
const outsideAnchorX = shelf.x - 60;
const outsideDividerSnap = snapBoard(state, divider, outsideAnchorX - divider.w / 2 + 6, divider.y);
assert(outsideDividerSnap.x === outsideAnchorX - divider.w / 2, "layout anchors outside a board should stay valid snap targets");
