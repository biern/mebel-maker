...
const ui = {
  starterBtn: query<HTMLButtonElement>("#starterBtn"),
...
function createStarter(recordHistory = true): void {
  ...
  state.gridOriginX = x;
  state.gridOriginY = y;
  addBoard({ name: "Left side", x, y, w: t, h: outerH, kind: "upright", autoThickness: "width" }, false);
  addBoard({ name: "Right side", x: x + outerW - t, y, w: t, h: outerH, kind: "upright", autoThickness: "width" }, false);
  addBoard({ name: "Top", x: x + t, y, w: outerW - 2 * t, h: t, kind: "shelf", autoThickness: "height" }, false);
  addBoard({ name: "Bottom", x: x + t, y: y + outerH - t, w: outerW - 2 * t, h: t, kind: "shelf", autoThickness: "height" }, false);
  addBoard({ name: "Middle shelf", x: x + t, y: y + 275, w: outerW - 2 * t, h: t, kind: "shelf", autoThickness: "height" }, false);
  state.selectedId = 1;
  fitToView();
}
...
ui.starterBtn.addEventListener("click", () => createStarter());
...
ui.gridInput.addEventListener("input", () => {
  remember();
  state.grid = Math.max(5, Number(ui.gridInput.value) || 25);
  refresh();
});
