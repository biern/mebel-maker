...
function updateInspector(): void {
  ...
  if (multi) {
    ...
    setInputValue(ui.yInput, selectionBounds ? String(Math.round(selectionBounds.top)) : null);
...
function updateBoardFromInspector(event?: Event): void {
  ...
  if (boards.length > 1) {
    ...
    if (selectionBounds && (target === ui.xInput || target === ui.yInput)) {
      ...
      const nextY = target === ui.yInput ? Number(ui.yInput.value) : selectionBounds.top;
